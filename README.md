...
真实持仓 SQLite 只以 Windows prod 为主数据；不要用 Git 同步 `data/portfolio.sqlite3`。

## 轻量 Monitor / Alias

Windows `quant-ai-web` 常驻服务会启动轻量 monitor：只监控真实持仓价格风险和持仓/观察池高价值新闻，不做公司级 data platform。

常用命令：

```bash
bin/quant-ai-local alias-list --config config/default.yaml
bin/quant-ai-local alias-set SNXX NVDA --config config/default.yaml --note "broker symbol mapping"
bin/quant-ai-local monitor-once --config config/default.yaml
bin/quant-ai-local monitor-status --config config/default.yaml
```

如果券商符号无法在 FMP/yfinance/Stooq 找到，先用 `alias-set` 映射到数据源 symbol。Monitor 会记录 data health、最近价格快照和新闻去重状态。
